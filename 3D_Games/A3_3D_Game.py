...
def showScreen():
    ...
    for x in range(-GRID_LENGTH, GRID_LENGTH, step):
        glBegin(GL_QUADS)
        # Alternate brightness for a "barcode" ghost effect
        intensity = 0.2 if (x // step) % 2 == 0 else 0.5
        glColor3f(intensity * pulse, 0, intensity)
        
        glVertex3f(x, GRID_LENGTH, 0)
        glVertex3f(x + 10, GRID_LENGTH, 0) # Thin strip
        
        glColor3f(0, 0, 0) # Fade to top
        glVertex3f(x + 10, GRID_LENGTH, height_z)
        glVertex3f(x, GRID_LENGTH, height_z)
        glEnd()
    draw_shapes()
